...
class menuFrame extends JDialog {
	...
	menuFrame() {
		menu = new JMenuBar();
		file = new JMenu("File");
		fnew = new JMenuItem("New			Ctrl+N");
		fsave = new JMenuItem("Save 			Ctrl+S");
		fopen = new JMenuItem("Open 			Ctrl+O");
		fexit = new JMenuItem("Exit");
		edit = new JMenu("Edit");
		eundo = new JMenuItem("Undo 		Ctrl+Z");
		eredo = new JMenuItem("Redo			Ctrl+R");
		esound = new JMenuItem("Sound");
		son = new JCheckBoxMenuItem("ON");
		soff = new JCheckBoxMenuItem("OFF");
		help = new JMenu("Help");
...
class dpanel extends JPanel {
	...
	public dpanel(int[] dikst, int mm, Point[] ppp, int nod, int[][] adjm) {
		...
		adm = adjm;
		min = mm;
		for (int i = 0; i < diks.length; i++) //System.out.println(diks[i]);
		{
			this.setPreferredSize(new Dimension(500, 400));
		}
		this.add(l1);
